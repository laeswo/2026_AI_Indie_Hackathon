...
public class Throw_trajectory : MonoBehaviour
{
    ...
    // 점선 텍스처 같은 머티리얼을 넣을 수 있다. 비우면 Sprites/Default 단색 선. 넣으면 텍스처가 길이 방향으로 반복된다.
    ...
    void Awake()
    {
        ...
        // 슬롯에 머티리얼을 넣었으면 그걸 쓰고(점선은 Tile 이라야 반복된다), 비었고 라인에도 없으면 기본값을 만든다. 분홍 선 방지.
        ...
        // 레이어 마스크는 인스펙터에서 바꿀 수 있으므로 레이캐스트 직전에 매번 넣는다.
