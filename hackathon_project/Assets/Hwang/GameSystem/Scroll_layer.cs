...
public class Scroll_layer : MonoBehaviour
{
    ...
    // 한 장 걸러 좌우를 뒤집어 붙인다. 양 끝이 안 맞는 그림도 이음새 없이 이어진다.
    // 되돌릴 때 짝이 유지되도록 타일 수를 짝수로 맞춘다.
    public bool mirror_alternate = false;

    int extra_tiles = 1;     // 화면을 채우고도 남길 여유 타일 수
    ...
    void Build()
    {
        ...
        SpriteRenderer original = tile.GetComponentInChildren<SpriteRenderer>();
        bool base_flip = original != null && original.flipX;
        ...
        for (int i = 1; i < count; i++) {
            ...
            // 홀수 번째만 뒤집는다. 원본(0번)과 번갈아 거울처럼 이어진다.
            if (mirror_alternate) {
                SpriteRenderer renderer = copy.GetComponentInChildren<SpriteRenderer>();
                if (renderer != null) {
                    renderer.flipX = (i % 2 == 1) ? !base_flip : base_flip;
                }
            }
        }
