...
// 아직 그림이 없는 오브젝트에 임시로 씌울 원형 스프라이트.
// SpriteRenderer 에 진짜 스프라이트가 들어 있으면 아무것도 하지 않는다.
public static class Placeholder_sprite
{
    ...
    // 스프라이트가 없을 때만 임시 원을 씌운다. 있으면 색과 크기도 건드리지 않는다.
    public static SpriteRenderer Ensure(GameObject target, Color color, float diameter, int sorting_order)
    {
        SpriteRenderer sprite_renderer = target.GetComponent<SpriteRenderer>();
        if (sprite_renderer == null) {
            sprite_renderer = target.AddComponent<SpriteRenderer>();
        }
